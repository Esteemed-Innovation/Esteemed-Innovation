...
public class TileEntitySteamCharger extends SteamTransporterTileEntity implements ISteamTransporter, IInventory {
        ...
        if (this.getStackInSlot(0).getItem() instanceof ISteamChargable) {
          ...
          if (!(item instanceof ItemExosuitArmor)) {
            ...
            if (this.getSteamShare() > item.steamPerDurability() && stack.getItemDamage() > 0) {
              int i = 0;
              while (i < 4
                && (this.getSteamShare() > item.steamPerDurability() && stack.getItemDamage() > 0)) {
          ...
          if (!tags.getCompoundTag("InfiTool").getBoolean("Broken")) {
            ...
            if (this.getSteamShare() > 8
              ...
              float currentPerc =
                (float) (tags.getCompoundTag("InfiTool").getInteger("TotalDurability") - tags
                  .getCompoundTag("InfiTool").getInteger("Damage"))
  ...
  public void dropItem(ItemStack item) {
    EntityItem entityItem =
      new EntityItem(this.worldObj, this.xCoord + 0.5F, this.yCoord + 1.25F, this.zCoord + 0.5F,
        item);
